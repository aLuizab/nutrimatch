...
export default async function DashboardProfissional({
  ...
  const [allAppointments, todayAppointments, monthRevenue, activeEnrollments] = await Promise.all([
    ...
    prisma.enrollment.count({ where: { professionalId, status: 'ACTIVE', endsAt: { gte: new Date() } } }),
  ])
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              {activeEnrollments === 0 ? (
                <p className="text-sm text-gray-500 leading-relaxed">
                  Você ainda não tem pacientes em acompanhamento.{' '}
                  <Link href="/programas" className="text-emerald-600 font-medium hover:underline">
                    Crie um programa
                  </Link>{' '}
                  para transformar consultas avulsas em relações contínuas.
                </p>
              ) : (
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-bold text-gray-900">{activeEnrollments}</span>
                  <span className="text-sm text-gray-500">
                    {activeEnrollments === 1 ? 'paciente em acompanhamento' : 'pacientes em acompanhamento'}
                  </span>
                </div>
              )}
            </div>
